% load('/Users/benjamin/Documents/python/calibrations/mat/20180316_004_caldata.mat'); data.caldata = caldata; clear caldata
load('/Users/benjamin/Documents/python/calibrations/mat/20180323_003_caldata.mat'); data.caldata = caldata; clear caldata

if strcmp(meta.date, '20180323/') && strcmp(meta.run, '003') && (length(data.ugvStereo.leftraw.time)==34)
    data.ugvStereo.leftraw.time(26) = [];
    data.ugvStereo.rightraw.time(26) = [];
end
...
% Spline Gator data
[data.caldata.gator_py_.q_x.spline, data.caldata.gator_py_.q_x.atRawtime]  = splineAndInterp(data.vicon.gator_py_.time, data.vicon.gator_py_.q_i(:,1), data.ugvStereo.leftraw.time);
[data.caldata.gator_py_.q_y.spline, data.caldata.gator_py_.q_y.atRawtime]  = splineAndInterp(data.vicon.gator_py_.time, data.vicon.gator_py_.q_i(:,2), data.ugvStereo.leftraw.time);
[data.caldata.gator_py_.q_z.spline, data.caldata.gator_py_.q_z.atRawtime]  = splineAndInterp(data.vicon.gator_py_.time, data.vicon.gator_py_.q_i(:,3), data.ugvStereo.leftraw.time);
[data.caldata.gator_py_.q_w.spline, data.caldata.gator_py_.q_w.atRawtime]  = splineAndInterp(data.vicon.gator_py_.time, data.vicon.gator_py_.q_i(:,4), data.ugvStereo.leftraw.time);
 data.caldata.gator_py_.q.atRawtime = [data.caldata.gator_py_.q_x.atRawtime data.caldata.gator_py_.q_y.atRawtime data.caldata.gator_py_.q_z.atRawtime data.caldata.gator_py_.q_w.atRawtime];
 
[data.caldata.gator_py_.P_x.spline, data.caldata.gator_py_.P_x.atRawtime]  = splineAndInterp(data.vicon.gator_py_.time, data.vicon.gator_py_.P.vicon(:,1), data.ugvStereo.leftraw.time);
[data.caldata.gator_py_.P_y.spline, data.caldata.gator_py_.P_y.atRawtime]  = splineAndInterp(data.vicon.gator_py_.time, data.vicon.gator_py_.P.vicon(:,2), data.ugvStereo.leftraw.time);
[data.caldata.gator_py_.P_z.spline, data.caldata.gator_py_.P_z.atRawtime]  = splineAndInterp(data.vicon.gator_py_.time, data.vicon.gator_py_.P.vicon(:,3), data.ugvStereo.leftraw.time);
 data.caldata.gator_py_.P.atRawtime = [data.caldata.gator_py_.P_x.atRawtime data.caldata.gator_py_.P_y.atRawtime data.caldata.gator_py_.P_z.atRawtime];
...
%     tl = data.caldata.leftposes.t.(matlab.lang.makeValidName(['t' num2str(i)]));
    Rl = cv2rodrigues(data.caldata.LeftRectPoses.rvecs(i,:))';
    tl = data.caldata.LeftRectPoses.tvecs(i,:)';
...
%     tr = data.caldata.rightposes.t.(matlab.lang.makeValidName(['t' num2str(i)]));
    Rr = cv2rodrigues(data.caldata.RightRectPoses.rvecs(i,:))';
    tr = data.caldata.LeftRectPoses.tvecs(i,:)';
    
%     data.caldata.Hlcam2gator(:,:,i) = Ryh(pi)*wrapH(Rl, tl);
%     data.caldata.Hrcam2gator(:,:,i) = Ryh(pi)*wrapH(Rr, tr);
    data.caldata.Rlcam2gator(:,:,i) = Rl;
    data.caldata.Hlcam2gator(:,:,i) = wrapH(Rl, tl);
    data.caldata.Rrcam2gator(:,:,i) = Rr;
    data.caldata.Hrcam2gator(:,:,i) = wrapH(Rr, tr);
    ...
    gator_py_q = data.caldata.gator_py_.q.atRawtime(i,:);
    data.caldata.gator_py_.dcm(:,:,i) = QwedgeDCM(gator_py_q);
    data.caldata.gator_py_.H_vic2lo(:,:,i) = wrapH(data.caldata.gator_py_.dcm(:,:,i), data.caldata.gator_py_.P.atRawtime(i,:)');
    
    data.caldata.Hgator2ugv(:,:,i) = data.caldata.ugvk.H_vic2lo(:,:,i) * invertH(data.caldata.gator_py_.H_vic2lo(:,:,i));
    data.caldata.Hugv2gator(:,:,i) = invertH(data.caldata.Hgator2ugv(:,:,i));
    
	delta(i,:) = data.caldata.ugvk.P.atRawtime(i,:) - data.caldata.gator_py_.P.atRawtime(i,:);
    rms_delta(i,1) = sqrt(delta(i,:) * delta(i,:)');

    
    % combine into camera to ugv (this is where the natural y is up rotation happens for the cameras)
...
figure(5000); clf; current_fig = gcf; disp(['figure(' num2str(current_fig.Number) ') ..']); clear current_fig
    hold on
    for i = 1:length(data.ugvStereo.leftraw.time)
%     for i = length(data.ugvStereo.leftraw.time)
    ...
    xaxis = [data.caldata.gator_py_.P.atRawtime(end,:)' data.caldata.gator_py_.P.atRawtime(end,:)'+0.25*data.caldata.gator_py_.dcm(1,:,end)'];
    yaxis = [data.caldata.gator_py_.P.atRawtime(end,:)' data.caldata.gator_py_.P.atRawtime(end,:)'+0.25*data.caldata.gator_py_.dcm(2,:,end)'];
    zaxis = [data.caldata.gator_py_.P.atRawtime(end,:)' data.caldata.gator_py_.P.atRawtime(end,:)'+0.25*data.caldata.gator_py_.dcm(3,:,end)'];
...
%%
meanred = mean(data.vicon.red_.splines.err_ugv(8:end,:))
meanblue = mean(data.vicon.blue_.splines.err_ugv(8:end,:))
meangreen = mean(data.vicon.green_.splines.err_ugv(8:end,:))

meanx = (meanred(1) + meanblue(1) + meangreen(1))/3
meany = (meanred(2) + meanblue(2) + meangreen(2))/3
meanz = (meanred(3) + meanblue(3) + meangreen(3))/3

%%
ans =
    0.0427    0.0247    0.2106


ans =
    0.0506    0.0231    0.2085
    

ans =
    0.0572    0.0190    0.2139
