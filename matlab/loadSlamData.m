function data = loadSlamData(meta)
...
%% synchronize ros times
    try
        ...
        timers.SimpleCells = {...
            ...
            sprintf('data.%s.pgryaml.Guide.time',meta.ugv_leader_stereo); ...
...
%% Set csv file names
    ...
    vicon.april_list = april_csv_list;
    
%% Set up "global" from ugv initial location
        i = 1;
        vicon.global.origin = vicon.(matlab.lang.makeValidName(meta.ugv_leader)).P.vicon(i,:);
        ...
        try
            vicon.global.origin = vicon.(matlab.lang.makeValidName(meta.ugv_leader)).P.vicon(global_range(1),:);
            vicon.global.orientation = mean(vicon.(matlab.lang.makeValidName(meta.ugv_leader)).yaw.radians(global_range(1):global_range(2)));  % radians
...
%% Calculate uav yaw from vicon data (in vicon frame?)
        ...
        try [vicon] = yawfromviconRGB(vicon); catch; warning(['issue with [vicon] = yawfromviconRGB(vicon);']); end
        
%% calculate ugv (and globalrelative positions of markers
        try
            P_red = vicon.red_.P.vicon - vicon.(matlab.lang.makeValidName(meta.ugv_leader)).P.vicon;
            P_blue = vicon.blue_.P.vicon - vicon.(matlab.lang.makeValidName(meta.ugv_leader)).P.vicon;
            P_green = vicon.green_.P.vicon - vicon.(matlab.lang.makeValidName(meta.ugv_leader)).P.vicon;
            for i = 1:size(P_red,1)
                vicon.red_.P.ugv(i,:) = vicon.(matlab.lang.makeValidName(meta.ugv_leader)).R_vic2lo(:,:,i) * P_red(i,:)';
                vicon.blue_.P.ugv(i,:) = vicon.(matlab.lang.makeValidName(meta.ugv_leader)).R_vic2lo(:,:,i) * P_blue(i,:)';
                vicon.green_.P.ugv(i,:) = vicon.(matlab.lang.makeValidName(meta.ugv_leader)).R_vic2lo(:,:,i) * P_green(i,:)';
            end
...
%             Rcam2ugv = Rz(-pi/2)*Rx(-(90-21.5)*pi/180);
            try 
                Hcam2ugv = [Rcam2ugv ...
                    [eval(sprintf('data.%s.pgryaml.camXOffset',meta.ugv_leader_stereo)) ...
                     eval(sprintf('data.%s.pgryaml.camYOffset',meta.ugv_leader_stereo)) ...
                     eval(sprintf('data.%s.pgryaml.camZOffset',meta.ugv_leader_stereo))]';[0 0 0 1]];
            catch
            end
            try 
                Hcam2ugv = eval(sprintf('data.%s.cam2ugv',meta.ugv_leader_stereo));
            catch
...
%             catch;  disp("catch::Calculate camera-frame positions of markers"); end
...
% I think this code is used to generate one of the vecotr used to compute offset correlations
    try 
        x = data.ugvRecorder.wheel.quat(:,1);
        y = data.ugvRecorder.wheel.quat(:,2);
        z = data.ugvRecorder.wheel.quat(:,3);
        w = data.ugvRecorder.wheel.quat(:,4);
        data.ugvRecorder.wheel.yaw = -atan2(2*(x.*y - z.*w ) , 1 - 2*(y.^2 + z.^2)); %atan2(2,1);
        clear x y z w
    catch
        disp("catch:: x = data.ugvRecorder.wheel.quat(:,1);")
    end

% set vicon time using vicon.offset
    [vicon.syncoffset] = viconTimerSync(data.ugvRecorder, vicon, eval(sprintf('data.%s',meta.ugv_leader_stereo)), data.april, data.uavRecorder);
    ...
    try 
        for n = 1:length(vicon.TimerCell)
            ...
            if isfield(vicon, timerstring)
                ...
                data.timers.StartTime = min([data.timers.StartTime vicon.(genvarname(timerstring)).time(1)]);
                data.timers.EndTime = max([data.timers.EndTime vicon.(genvarname(timerstring)).time(1)]);
            end
        end
    catch
        disp("catch:: for n = 1:length(vicon.TimerCell)")
    end
...
%     vicon = viconsplines(vicon, ugvStereo, uavRecorder, ugvRecorder, meta);
    ...
    try 
        vicon = viconsplines(vicon, data, meta); 
    catch; warning('viconsplines failed, skipping ..'); end
    try
        for i=1:length(ugvStereo.uav.P_ugv)
            ugvStereo.uav.P_gl(i,:) = (vicon.(matlab.lang.makeValidName(meta.ugv_leader)).splines.H_stereotime.lo2gl(:,:,i)*[ugvStereo.uav.P_ugv(i,:) 1]')';
        end
    catch; disp('ugvStereo.uav.P_gl failed'); end
    
    try 
        tagnames = fieldnames(data.april);
    catch; disp(['   ** tagnames = fieldnames(data.april); failed.']); end
...
%                 vicon.(matlab.lang.makeValidName(meta.ugv_leader)).time, vicon.(matlab.lang.makeValidName(meta.ugv_leader)).yaw.global*180/pi);
%             i = i + 1;
%     catch
%     end
% 
...
%     try vicon.diff_yaw.uav_ugv = differentialYaw(vicon.(matlab.lang.makeValidName(meta.ugv_leader)).x_axis, vicon.uav.x_axis(:,1:3)); catch; end
...
function vicon = viconsplines(vicon, data, meta)
%% Spline the ugv positions of uav markers

    try
    [data.uavRecorder.navdata.compassSpline, ...
        data.uavRecorder.navdata.vicon_yaw, ...
        data.uavRecorder.navdata.compassError] = ... 
            spliner(vicon.uav.time, vicon.uav.yaw.degrees, ...
                    data.uavRecorder.navdata.time, data.uavRecorder.navdata.CompassYaw); 
    catch; warning([data.uavRecorder.navdata.compassSpline]);  end

    spline = csapi(vicon.(matlab.lang.makeValidName(meta.ugv_leader)).time,vicon.(matlab.lang.makeValidName(meta.ugv_leader)).yaw.global);
        vector_newtime = fnval(spline, data.ugvRecorder.stereo.time);
            vicon.(matlab.lang.makeValidName(meta.ugv_leader)).atStereoTime.yaw.global = vector_newtime;
    ...
    vicon.red_.splines.P.ugv.x_ugvStereotime = fnval(vicon.red_.splines.P.ugv.x, data.(matlab.lang.makeValidName(meta.ugv_leader_stereo)).time);
    vicon.red_.splines.P.ugv.y_ugvStereotime = fnval(vicon.red_.splines.P.ugv.y, data.(matlab.lang.makeValidName(meta.ugv_leader_stereo)).time);
    vicon.red_.splines.P.ugv.z_ugvStereotime = fnval(vicon.red_.splines.P.ugv.z, data.(matlab.lang.makeValidName(meta.ugv_leader_stereo)).time);    
    vicon.red_.splines.P.ugv.x_diff = vicon.red_.splines.P.ugv.x_ugvStereotime - data.(matlab.lang.makeValidName(meta.ugv_leader_stereo)).Red.P_ugv(:,1);
    vicon.red_.splines.P.ugv.y_diff = vicon.red_.splines.P.ugv.y_ugvStereotime - data.(matlab.lang.makeValidName(meta.ugv_leader_stereo)).Red.P_ugv(:,2);
    vicon.red_.splines.P.ugv.z_diff = vicon.red_.splines.P.ugv.z_ugvStereotime - data.(matlab.lang.makeValidName(meta.ugv_leader_stereo)).Red.P_ugv(:,3);
    ...
    vicon.blue_.splines.P.ugv.x_ugvStereotime = fnval(vicon.blue_.splines.P.ugv.x, data.(matlab.lang.makeValidName(meta.ugv_leader_stereo)).time);
    vicon.blue_.splines.P.ugv.y_ugvStereotime = fnval(vicon.blue_.splines.P.ugv.y, data.(matlab.lang.makeValidName(meta.ugv_leader_stereo)).time);
    vicon.blue_.splines.P.ugv.z_ugvStereotime = fnval(vicon.blue_.splines.P.ugv.z, data.(matlab.lang.makeValidName(meta.ugv_leader_stereo)).time);
    vicon.blue_.splines.P.ugv.x_diff = vicon.blue_.splines.P.ugv.x_ugvStereotime - data.(matlab.lang.makeValidName(meta.ugv_leader_stereo)).Blue.P_ugv(:,1);
    vicon.blue_.splines.P.ugv.y_diff = vicon.blue_.splines.P.ugv.y_ugvStereotime - data.(matlab.lang.makeValidName(meta.ugv_leader_stereo)).Blue.P_ugv(:,2);
    vicon.blue_.splines.P.ugv.z_diff = vicon.blue_.splines.P.ugv.z_ugvStereotime - data.(matlab.lang.makeValidName(meta.ugv_leader_stereo)).Blue.P_ugv(:,3);
    ...
    vicon.green_.splines.P.ugv.x_ugvStereotime = fnval(vicon.green_.splines.P.ugv.x, data.(matlab.lang.makeValidName(meta.ugv_leader_stereo)).time);
    vicon.green_.splines.P.ugv.y_ugvStereotime = fnval(vicon.green_.splines.P.ugv.y, data.(matlab.lang.makeValidName(meta.ugv_leader_stereo)).time);
    vicon.green_.splines.P.ugv.z_ugvStereotime = fnval(vicon.green_.splines.P.ugv.z, data.(matlab.lang.makeValidName(meta.ugv_leader_stereo)).time);
    vicon.green_.splines.P.ugv.x_diff = vicon.green_.splines.P.ugv.x_ugvStereotime - data.(matlab.lang.makeValidName(meta.ugv_leader_stereo)).Green.P_ugv(:,1);
    vicon.green_.splines.P.ugv.y_diff = vicon.green_.splines.P.ugv.y_ugvStereotime - data.(matlab.lang.makeValidName(meta.ugv_leader_stereo)).Green.P_ugv(:,2);
    vicon.green_.splines.P.ugv.z_diff = vicon.green_.splines.P.ugv.z_ugvStereotime - data.(matlab.lang.makeValidName(meta.ugv_leader_stereo)).Green.P_ugv(:,3);
% catch
% end
...
%% Spline the cam positions of uav markers
    ...
    vicon.red_.splines.P.cam.x_ugvStereotime = fnval(vicon.red_.splines.P.cam.x, data.(matlab.lang.makeValidName(meta.ugv_leader_stereo)).time);
    vicon.red_.splines.P.cam.y_ugvStereotime = fnval(vicon.red_.splines.P.cam.y, data.(matlab.lang.makeValidName(meta.ugv_leader_stereo)).time);
    vicon.red_.splines.P.cam.z_ugvStereotime = fnval(vicon.red_.splines.P.cam.z, data.(matlab.lang.makeValidName(meta.ugv_leader_stereo)).time);    
    vicon.red_.splines.P.cam.x_diff = vicon.red_.splines.P.cam.x_ugvStereotime - data.(matlab.lang.makeValidName(meta.ugv_leader_stereo)).Red.P_cam(:,1);
    vicon.red_.splines.P.cam.y_diff = vicon.red_.splines.P.cam.y_ugvStereotime - data.(matlab.lang.makeValidName(meta.ugv_leader_stereo)).Red.P_cam(:,2);
    vicon.red_.splines.P.cam.z_diff = vicon.red_.splines.P.cam.z_ugvStereotime - data.(matlab.lang.makeValidName(meta.ugv_leader_stereo)).Red.P_cam(:,3);
    ...
    vicon.blue_.splines.P.cam.x_ugvStereotime = fnval(vicon.blue_.splines.P.cam.x, data.(matlab.lang.makeValidName(meta.ugv_leader_stereo)).time);
    vicon.blue_.splines.P.cam.y_ugvStereotime = fnval(vicon.blue_.splines.P.cam.y, data.(matlab.lang.makeValidName(meta.ugv_leader_stereo)).time);
    vicon.blue_.splines.P.cam.z_ugvStereotime = fnval(vicon.blue_.splines.P.cam.z, data.(matlab.lang.makeValidName(meta.ugv_leader_stereo)).time);
    vicon.blue_.splines.P.cam.x_diff = vicon.blue_.splines.P.cam.x_ugvStereotime - data.(matlab.lang.makeValidName(meta.ugv_leader_stereo)).Blue.P_cam(:,1);
    vicon.blue_.splines.P.cam.y_diff = vicon.blue_.splines.P.cam.y_ugvStereotime - data.(matlab.lang.makeValidName(meta.ugv_leader_stereo)).Blue.P_cam(:,2);
    vicon.blue_.splines.P.cam.z_diff = vicon.blue_.splines.P.cam.z_ugvStereotime - data.(matlab.lang.makeValidName(meta.ugv_leader_stereo)).Blue.P_cam(:,3);
    ...
    vicon.green_.splines.P.cam.x_ugvStereotime = fnval(vicon.green_.splines.P.cam.x, data.(matlab.lang.makeValidName(meta.ugv_leader_stereo)).time);
    vicon.green_.splines.P.cam.y_ugvStereotime = fnval(vicon.green_.splines.P.cam.y, data.(matlab.lang.makeValidName(meta.ugv_leader_stereo)).time);
    vicon.green_.splines.P.cam.z_ugvStereotime = fnval(vicon.green_.splines.P.cam.z, data.(matlab.lang.makeValidName(meta.ugv_leader_stereo)).time);
    vicon.green_.splines.P.cam.x_diff = vicon.green_.splines.P.cam.x_ugvStereotime - data.(matlab.lang.makeValidName(meta.ugv_leader_stereo)).Green.P_cam(:,1);
    vicon.green_.splines.P.cam.y_diff = vicon.green_.splines.P.cam.y_ugvStereotime - data.(matlab.lang.makeValidName(meta.ugv_leader_stereo)).Green.P_cam(:,2);
    vicon.green_.splines.P.cam.z_diff = vicon.green_.splines.P.cam.z_ugvStereotime - data.(matlab.lang.makeValidName(meta.ugv_leader_stereo)).Green.P_cam(:,3);
...
try 
    Cx = data.(matlab.lang.makeValidName(meta.ugv_leader_stereo)).Original.Cx; 
    Cy = data.(matlab.lang.makeValidName(meta.ugv_leader_stereo)).Original.Cy;
catch
    Cx = data.(matlab.lang.makeValidName(meta.ugv_leader_stereo)).Cx; 
    Cy = data.(matlab.lang.makeValidName(meta.ugv_leader_stereo)).Cy;
end


data.(matlab.lang.makeValidName(meta.ugv_leader_stereo)).Projection.Right(1,4) = -155;

for i = 1:length(vicon.red_.splines.P_cam)
    uvw_l = data.(matlab.lang.makeValidName(meta.ugv_leader_stereo)).Projection.Left*vicon.red_.splines.P_cam(i,:)'; uvw_l = uvw_l'/uvw_l(3);
    uvw_r = data.(matlab.lang.makeValidName(meta.ugv_leader_stereo)).Projection.Right*vicon.red_.splines.P_cam(i,:)'; uvw_r = uvw_r'/uvw_r(3);
    ...
    vicon.red_.splines.err_cam = vicon.red_.splines.P_cam(:,1:3) - data.(matlab.lang.makeValidName(meta.ugv_leader_stereo)).Red.P_cam;
    vicon.red_.splines.err_ugv = vicon.red_.splines.P_ugv(:,1:3) - data.(matlab.lang.makeValidName(meta.ugv_leader_stereo)).Red.P_ugv;
    ...
    uvw_l = data.(matlab.lang.makeValidName(meta.ugv_leader_stereo)).Projection.Left*vicon.blue_.splines.P_cam(i,:)'; uvw_l = uvw_l'/uvw_l(3);
    uvw_r = data.(matlab.lang.makeValidName(meta.ugv_leader_stereo)).Projection.Right*vicon.blue_.splines.P_cam(i,:)'; uvw_r = uvw_r'/uvw_r(3);
    ...
    vicon.blue_.splines.err_cam = vicon.blue_.splines.P_cam(:,1:3) - data.(matlab.lang.makeValidName(meta.ugv_leader_stereo)).Blue.P_cam;
    vicon.blue_.splines.err_ugv = vicon.blue_.splines.P_ugv(:,1:3) - data.(matlab.lang.makeValidName(meta.ugv_leader_stereo)).Blue.P_ugv;
    ...
    uvw_l = data.(matlab.lang.makeValidName(meta.ugv_leader_stereo)).Projection.Left*vicon.green_.splines.P_cam(i,:)'; uvw_l = uvw_l'/uvw_l(3);
    uvw_r = data.(matlab.lang.makeValidName(meta.ugv_leader_stereo)).Projection.Right*vicon.green_.splines.P_cam(i,:)'; uvw_r = uvw_r'/uvw_r(3);
    ...
    vicon.green_.splines.err_cam = vicon.green_.splines.P_cam(:,1:3) - data.(matlab.lang.makeValidName(meta.ugv_leader_stereo)).Green.P_cam;
    vicon.green_.splines.err_ugv = vicon.green_.splines.P_ugv(:,1:3) - data.(matlab.lang.makeValidName(meta.ugv_leader_stereo)).Green.P_ugv;
...
%% UAV pose splines
    ...
    vicon.uav.splines.P.cam.x_uavstereotime = fnval(vicon.uav.splines.P.cam.x, data.(matlab.lang.makeValidName(meta.ugv_leader_stereo)).time);
    vicon.uav.splines.P.cam.y_uavstereotime = fnval(vicon.uav.splines.P.cam.y, data.(matlab.lang.makeValidName(meta.ugv_leader_stereo)).time);
    vicon.uav.splines.P.cam.z_uavstereotime = fnval(vicon.uav.splines.P.cam.z, data.(matlab.lang.makeValidName(meta.ugv_leader_stereo)).time);
    vicon.uav.splines.P.cam.x_diff = vicon.uav.splines.P.cam.x_uavstereotime - data.(matlab.lang.makeValidName(meta.ugv_leader_stereo)).uav.Obs_cam(:,1);
    vicon.uav.splines.P.cam.y_diff = vicon.uav.splines.P.cam.y_uavstereotime - data.(matlab.lang.makeValidName(meta.ugv_leader_stereo)).uav.Obs_cam(:,2);
    vicon.uav.splines.P.cam.z_diff = vicon.uav.splines.P.cam.z_uavstereotime - data.(matlab.lang.makeValidName(meta.ugv_leader_stereo)).uav.Obs_cam(:,3);
    ...
    vicon.uav.splines.P.global.x_uavesttime = fnval(vicon.uav.splines.P.global.x, data.uavRecorder.est.time);
    vicon.uav.splines.P.global.y_uavesttime = fnval(vicon.uav.splines.P.global.y, data.uavRecorder.est.time);
    vicon.uav.splines.P.global.z_uavesttime = fnval(vicon.uav.splines.P.global.z, data.uavRecorder.est.time);
    vicon.uav.splines.P.global.x_diff = vicon.uav.splines.P.global.x_uavesttime - data.uavRecorder.est.Position_gl(:,1);
    vicon.uav.splines.P.global.y_diff = vicon.uav.splines.P.global.y_uavesttime - data.uavRecorder.est.Position_gl(:,2);
    vicon.uav.splines.P.global.z_diff = vicon.uav.splines.P.global.z_uavesttime - data.uavRecorder.est.Position_gl(:,3);

    % stereotime in global  frame
    vicon.uav.splines.P.global.x_uavStereotime = fnval(vicon.uav.splines.P.global.x, data.(matlab.lang.makeValidName(meta.ugv_leader_stereo)).time);
    vicon.uav.splines.P.global.y_uavStereotime = fnval(vicon.uav.splines.P.global.y, data.(matlab.lang.makeValidName(meta.ugv_leader_stereo)).time);
    vicon.uav.splines.P.global.z_uavStereotime = fnval(vicon.uav.splines.P.global.z, data.(matlab.lang.makeValidName(meta.ugv_leader_stereo)).time);
    vicon.uav.splines.P.global.x_diff_stereotime = vicon.uav.splines.P.global.x_uavStereotime - data.(matlab.lang.makeValidName(meta.ugv_leader_stereo)).uav.P_ugv(:,1);
    vicon.uav.splines.P.global.y_diff_stereotime = vicon.uav.splines.P.global.y_uavStereotime - data.(matlab.lang.makeValidName(meta.ugv_leader_stereo)).uav.P_ugv(:,2);
    vicon.uav.splines.P.global.z_diff_stereotime = vicon.uav.splines.P.global.z_uavStereotime - data.(matlab.lang.makeValidName(meta.ugv_leader_stereo)).uav.P_ugv(:,3);
...
if (strcmp('20170914/', meta.date))
    ...
    vicon.ugvc.splines.P.global.x_ugvcesttime = fnval(vicon.ugvc.splines.P.global.x, data.ugvRecorder.est.time);
    vicon.ugvc.splines.P.global.y_ugvcesttime = fnval(vicon.ugvc.splines.P.global.y, data.ugvRecorder.est.time);
    vicon.ugvc.splines.P.global.z_ugvcesttime = fnval(vicon.ugvc.splines.P.global.z, data.ugvRecorder.est.time);
    
    % Vicon position - est position
    vicon.ugvc.splines.P.global.x_diff = vicon.ugvc.splines.P.global.x_ugvcesttime - data.ugvRecorder.est.Position_gl(:,1);
    vicon.ugvc.splines.P.global.y_diff = vicon.ugvc.splines.P.global.y_ugvcesttime - data.ugvRecorder.est.Position_gl(:,2);
    vicon.ugvc.splines.P.global.z_diff = vicon.ugvc.splines.P.global.z_ugvcesttime - data.ugvRecorder.est.Position_gl(:,3);

    % yaw
    vicon.ugvc.splines.yaw.est = csapi(vicon.ugvc.time,vicon.ugvc.yaw.global); 
    vicon.ugvc.splines.yaw.yaw_ugvcesttime = fnval(vicon.ugvc.splines.yaw.est, data.ugvRecorder.est.time);
    vicon.ugvc.splines.yaw.est_err = vicon.ugvc.splines.yaw.yaw_ugvcesttime*180/pi - data.ugvRecorder.est.Yaw;
else
    disp('   splining ugvk data')
    % spline of 
    vicon.(matlab.lang.makeValidName(meta.ugv_leader)).splines.P.global.x = csapi(vicon.(matlab.lang.makeValidName(meta.ugv_leader)).time,vicon.(matlab.lang.makeValidName(meta.ugv_leader)).P.global(:,1));
    vicon.(matlab.lang.makeValidName(meta.ugv_leader)).splines.P.global.y = csapi(vicon.(matlab.lang.makeValidName(meta.ugv_leader)).time,vicon.(matlab.lang.makeValidName(meta.ugv_leader)).P.global(:,2));
    vicon.(matlab.lang.makeValidName(meta.ugv_leader)).splines.P.global.z = csapi(vicon.(matlab.lang.makeValidName(meta.ugv_leader)).time,vicon.(matlab.lang.makeValidName(meta.ugv_leader)).P.global(:,3));
    
    %Vicon data in ugvRecorder time
    vicon.(matlab.lang.makeValidName(meta.ugv_leader)).splines.P.global.x_ugvkesttime = fnval(vicon.(matlab.lang.makeValidName(meta.ugv_leader)).splines.P.global.x, data.ugvRecorder.est.time);
    vicon.(matlab.lang.makeValidName(meta.ugv_leader)).splines.P.global.y_ugvkesttime = fnval(vicon.(matlab.lang.makeValidName(meta.ugv_leader)).splines.P.global.y, data.ugvRecorder.est.time);
    vicon.(matlab.lang.makeValidName(meta.ugv_leader)).splines.P.global.z_ugvkesttime = fnval(vicon.(matlab.lang.makeValidName(meta.ugv_leader)).splines.P.global.z, data.ugvRecorder.est.time);
    vicon.(matlab.lang.makeValidName(meta.ugv_leader)).splines.P.global.x_ugvkstereotime = fnval(vicon.(matlab.lang.makeValidName(meta.ugv_leader)).splines.P.global.x, data.(matlab.lang.makeValidName(meta.ugv_leader_stereo)).time);
    vicon.(matlab.lang.makeValidName(meta.ugv_leader)).splines.P.global.y_ugvkstereotime = fnval(vicon.(matlab.lang.makeValidName(meta.ugv_leader)).splines.P.global.y, data.(matlab.lang.makeValidName(meta.ugv_leader_stereo)).time);
    vicon.(matlab.lang.makeValidName(meta.ugv_leader)).splines.P.global.z_ugvkstereotime = fnval(vicon.(matlab.lang.makeValidName(meta.ugv_leader)).splines.P.global.z, data.(matlab.lang.makeValidName(meta.ugv_leader_stereo)).time);
    
    % Vicon position - est position
    vicon.(matlab.lang.makeValidName(meta.ugv_leader)).splines.P.global.x_diff = vicon.(matlab.lang.makeValidName(meta.ugv_leader)).splines.P.global.x_ugvkesttime - data.ugvRecorder.est.Position_gl(:,1);
    vicon.(matlab.lang.makeValidName(meta.ugv_leader)).splines.P.global.y_diff = vicon.(matlab.lang.makeValidName(meta.ugv_leader)).splines.P.global.y_ugvkesttime - data.ugvRecorder.est.Position_gl(:,2);
    vicon.(matlab.lang.makeValidName(meta.ugv_leader)).splines.P.global.z_diff = vicon.(matlab.lang.makeValidName(meta.ugv_leader)).splines.P.global.z_ugvkesttime - data.ugvRecorder.est.Position_gl(:,3);

    % yaw
    vicon.(matlab.lang.makeValidName(meta.ugv_leader)).splines.yaw.est = csapi(vicon.(matlab.lang.makeValidName(meta.ugv_leader)).time,vicon.(matlab.lang.makeValidName(meta.ugv_leader)).yaw.global); 
    vicon.(matlab.lang.makeValidName(meta.ugv_leader)).splines.yaw.yaw_ugvkesttime = fnval(vicon.(matlab.lang.makeValidName(meta.ugv_leader)).splines.yaw.est, data.ugvRecorder.est.time);
    vicon.(matlab.lang.makeValidName(meta.ugv_leader)).splines.yaw.est_err = vicon.(matlab.lang.makeValidName(meta.ugv_leader)).splines.yaw.yaw_ugvkesttime*180/pi - data.ugvRecorder.est.Yaw;
    vicon.(matlab.lang.makeValidName(meta.ugv_leader)).splines.yaw.yaw_ugvkstereotime = fnval(vicon.(matlab.lang.makeValidName(meta.ugv_leader)).splines.yaw.est, data.(matlab.lang.makeValidName(meta.ugv_leader_stereo)).time);
    
    for i = 1:length(vicon.(matlab.lang.makeValidName(meta.ugv_leader)).splines.yaw.yaw_ugvkstereotime)
        theta = vicon.(matlab.lang.makeValidName(meta.ugv_leader)).splines.yaw.yaw_ugvkstereotime(i);
        t = [vicon.(matlab.lang.makeValidName(meta.ugv_leader)).splines.P.global.x_ugvkstereotime(i) ...
             vicon.(matlab.lang.makeValidName(meta.ugv_leader)).splines.P.global.y_ugvkstereotime(i) ...
             vicon.(matlab.lang.makeValidName(meta.ugv_leader)).splines.P.global.z_ugvkstereotime(i)]';
        vicon.(matlab.lang.makeValidName(meta.ugv_leader)).splines.H_stereotime.lo2gl(:,:,i) = wrapH(Rz(theta)', -t);
    end
end
%% Compute yaw splines of vicon data

    vicon.uav.splines.yaw.global.degrees = csapi(vicon.uav.time, vicon.uav.yaw.global.degrees);
    vicon.uav.splines.yaw.global.degrees_stereotime = fnval(vicon.uav.splines.yaw.global.degrees, data.(matlab.lang.makeValidName(meta.ugv_leader_stereo)).time);
    vicon.uav.splines.yaw.global.error_stereotime =  vicon.uav.splines.yaw.global.degrees_stereotime - data.(matlab.lang.makeValidName(meta.ugv_leader_stereo)).uav.yaw_ugv;
...
%     vicon.diff_yaw.spline_var = var(vicon.diff_yaw.spline_error);
    ...
    vicon.est_yaw.splineyaw = fnval(vicon.est_yaw.spline, data.uavRecorder.est.time);
    vicon.est_yaw.spline_error = data.uavRecorder.est.Yaw - vicon.est_yaw.splineyaw;
    ...
    vicon.uav.yaw.global.degrees_error = data.uavRecorder.est.Yaw - vicon.uav.splines.yaw.global.deg_atEstTime;
end
