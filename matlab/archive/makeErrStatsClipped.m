function stats = makeErrStatsClipped(vicon, uavRecorder, meta, run, experiment)
...
%% figure(100x); clf; current_fig = gcf; disp(['figure(' num2str(current_fig.Number) ') ..']); clear current_fig
    ...
    subplot (2,1,1)
        ...
        xlabel('time [s]')
        ylabel('error [m]')
        title(['uav x y z yaw error vs time ' meta.date(1:end-1)  ' ' meta.run])
    subplot (2,1,2)
...
%% figure(200x); clf; current_fig = gcf; disp(['figure(' num2str(current_fig.Number) ') ..']); clear current_fig
    ...
    subplot (2,1,1)
        ...
        xlabel('range [m]')
        ylabel('error [m]')
        title(['uav x y z yaw error vs optical range ' meta.date(1:end-1)  ' ' meta.run])
