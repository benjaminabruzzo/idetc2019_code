...
class gazebo_logger():
	...
	def __init__(self):
		...
		print "gazebo_logger:: trial:= " + str_trial
		
		self.file_loggerfilename =('/home/benjamin/ros/data/csv/hast_{0}_{1}.csv').format(str_date,str_trial)
		...
		self.cfg_loggername = ('/home/benjamin/ros/data/{0}/{1}/hast_cfg_{1}.m').format(str_date, str_trial)
		self.cfg_logger = open(self.cfg_loggername, 'w')
		self.cfg_logger.write("%% "+str_date+'\n')
		self.cfg_logger.write("%% "+str_trial+'\n')

		self.make_csv_headers()
		for tf_path in self.body_paths:
			print("%% gazebo_logger::logging tf:{} to tf:{}").format(str(self.world_origin), str(tf_path)) 
			self.cfg_logger.write(("%% gazebo_logger::logging tf:{} to tf:{}\n").format(str(self.world_origin), str(tf_path)))

		self.xf_listener = tf.TransformListener()

		record_pid = rospy.get_param('~record_pid')
		if record_pid:
			self.pid_filename = '/home/benjamin/ros/data/' + str_date + '/' + str_trial + '/pid_' + str_trial + '.m'
			...
			self.pid_filealloc = '/home/benjamin/ros/data/' + str_date + '/' + str_trial + '/pid_prealloc_' + str_trial + '.m'
		else:
