...
/*---------Imagery Variables ------------- */
  ...
  /*--- KeyPoint Containers --- */
  cv::vector<cv::KeyPoint> lRedKeypoint, rRedKeypoint, lRedBlobpoint, rRedBlobpoint;
  cv::vector<cv::KeyPoint> lBlueKeypoint, rBlueKeypoint, lBlueBlobpoint, rBlueBlobpoint;
  cv::vector<cv::KeyPoint> lGreenKeypoint, rGreenKeypoint, lGreenBlobpoint, rGreenBlobpoint;
  ...
  cv::Mat closingElement; // = getStructuringElement(2,cv::Size( 7, 7 ),cv::Point(-1,-1));
    ...
    cv::vector<cv::vector<cv::Point> > leftGreenContours, leftBlueContours, leftRedContours;
    cv::vector<cv::vector<cv::Point> > rightGreenContours, rightBlueContours, rightRedContours;
    cv::vector<cv::vector<cv::Point> > lGreenPolygons, lBluePolygons, lRedPolygons;
    cv::vector<cv::vector<cv::Point> > rGreenPolygons, rBluePolygons, rRedPolygons;
    /*--- Green HSV --- */
...
public:
  ...
  : it(n)
  {
    ...
    if(n.getParam("/hast/stereo/numimages", numimages)){} else {numimages = 20;}

    LeftImage_sub    = it.subscribe("/pgrstereo/left/image_rect_color",   1,  &stereoObs::leftCvBridge, this);
    RightImage_sub   = it.subscribe("/pgrstereo/right/image_rect_color",  1,  &stereoObs::rightCvBridge, this);
    ...
    if(n.getParam("/hast/stereo/camXOffset", camXOffset)){} else {camXOffset = 0.055;}  // [m]
    if(n.getParam("/hast/stereo/camZOffset", camZOffset)){} else {camZOffset = 0.225;}  // [m]
  ...
  void leftCvBridge(const sensor_msgs::ImageConstPtr& msg)
  {
    ...
    try
    {
      ...
      // if(save_rect)
      // {
      //   s_imString = s_root + s_date + "/" + s_run + "/original/left_rect_" + patch::to_string(++leftrect_count) + ".png"; 
  ...
  void rightCvBridge(const sensor_msgs::ImageConstPtr& msg)
  {
    ...
    try
    {
      ...
      // if(save_rect)
      // {
      //   s_imString = s_root + s_date + "/" + s_run + "/original/right_rect_" + patch::to_string(++rightrect_count) + ".png"; 
      //   imwrite(s_imString.c_str(), cv_ptr->image);
      // }
    }
    catch (cv_bridge::Exception& e)
  ...
  {//begin calculate
    ...
    /*---------convert images to YUV ------------- */
      ...
      cv::vector<cv::Mat> lchannels(3);
      cv::vector<cv::Mat> rchannels(3);
  ...
  cv::Vec<double,4> testRadii(cv::vector<cv::KeyPoint> kpr, cv::vector<cv::KeyPoint> kpl, const char* color)
  {
    cv::Vec<double,4> pix;
  ...
  cv::Vec<double,4> testPix(cv::vector<cv::KeyPoint> kpr, cv::vector<cv::KeyPoint> kpl, const char* color)
  {
    cv::Vec<double,4> pix;
      if (kpr.size() == 1)
      {//begin if
  ...
  cv::vector<cv::vector<cv::Point> > getContours(cv::Mat src_image)
  {
  ...
  cv::vector<cv::vector<cv::Point> > getPolygons(cv::vector<cv::vector<cv::Point> > contours)
  {
    /// Approximate contours to polygons + get bounding rects and circles
    cv::vector<cv::vector<cv::Point> > polygons( contours.size() );
  ...
  void drawPolygons(cv::vector<cv::vector<cv::Point> > polygons, cv::Mat dest_image,cv::Scalar color )
  {
    for( size_t i = 0; i < polygons.size(); i++ )
    {cv::drawContours(dest_image, polygons, int(i), color , 1,8,cv::vector<cv::Vec4i>(), 0, cv::Point());}
  } //end drawPolygons

  cv::vector<cv::KeyPoint> getCircles(cv::vector<cv::vector<cv::Point> > polygons)
  {
    /// Approximate contours to polygons + get bounding rects and circles
    cv::vector<cv::KeyPoint> keypoints;
    cv::vector<cv::Point2f>center( polygons.size() );
    cv::vector<float>radius( polygons.size() );
    for( size_t i = 0; i < polygons.size(); i++ )
    {
  ...
  void drawCircles(cv::vector<cv::KeyPoint> keypoints, cv::Mat dest_image, cv::Scalar color )
  {
	...
	cv::vector<cv::KeyPoint> Blobbing(cv::Mat BlobD)
	{
	  /*--- Detect Blobs --- */
	...
	void drawBlobs(cv::vector<cv::KeyPoint> kpr, cv::vector<cv::KeyPoint> kpl, cv::Scalar color)
	{
		for (unsigned int i=0; i<kpr.size(); i++)
			{
