function Calibration = blackflyCal(HOSTID)
%% Host name
% clear all
% HOSTID = 'kobuki';
% HOSTID = 'create';
...
% 16306423 USB3
    ...
    Left.SERIAL  = '16452463';
    Right.SERIAL = '17094423';
%     HOSTUSER = 'turtlebot';
...
% fprintf(Launch.fileID, '\t\t\t<!-- <param name="frame_rate" value="$(arg frame_rate)" /> -->\n');
fprintf(Launch.fileID, '\t\t\t<param name="camera_info_url" if="$(arg left_camera_calibrated)" value="file://$(env HOME)/ros/src/hast/cam_info/$(arg left_camera_serial).yaml" /> \n');
...
fprintf(Launch.fileID, '\t\t<node pkg="nodelet" type="nodelet" name="%s_nodelet" args="load pointgrey_camera_driver/PointGreyCameraNodelet %s_nodelet_manager">\n', 'right', 'right');    
fprintf(Launch.fileID, '\t\t\t<param name="frame_id" value="%s" />\n', 'right');
fprintf(Launch.fileID, '\t\t\t<param name="serial" value="%s" />\n', Right.SERIAL);
% fprintf(Launch.fileID, '\t\t\t<!-- <param name="frame_rate" value="$(arg frame_rate)" /> -->\n');
fprintf(Launch.fileID, '\t\t\t<param name="camera_info_url" if="$(arg left_camera_calibrated)" value="file://$(env HOME)/ros/src/hast/cam_info/$(arg right_camera_serial).yaml" />\n');
fprintf(Launch.fileID, '\t\t</node>\n');
fprintf(Launch.fileID, '\t\t<node pkg="nodelet" type="nodelet" name="image_proc_debayer" args="load image_proc/debayer %s_nodelet_manager">\n', 'right');
fprintf(Launch.fileID, '\t\t</node>\n');
fprintf(Launch.fileID, '\t\t<node pkg="image_view" type="image_view" name="image_view" >\n');
...
% ----------- setBlackfly -----------
fprintf(Launch.fileID,'\n\n<!-- <node pkg="hast" type="setBlackfly" name="setBlackfly" output="screen"/> -->\n');
fprintf(Launch.fileID,'\n\n</launch>');
...
fprintf(Left.yamlID,'camera_info_url: file://$(env HOME)/ros/src/hast/cam_info/${NAME}.yaml\n\n');
...
fprintf(Right.yamlID,'camera_info_url: file://$(env HOME)/ros/src/hast/cam_info/${NAME}.yaml\n\n');
