function [w,x,y,z]=FileExchange_euler2quat(rx,ry,rz,order)
...
%    disp('order is x-->y-->z');
    Q1=FileExchange_quaternionmul(Qy,Qz);  
    Q2=FileExchange_quaternionmul(Qx,Q1);  
    
elseif(strcmpi(order,'xzy')==1)  
 %   disp('order is x-->z-->y');
    Q1=FileExchange_quaternionmul(Qz,Qy);  
    Q2=FileExchange_quaternionmul(Qx,Q1);
    
elseif(strcmpi(order,'yxz')==1)
  %  disp('order is y-->x-->z');
    Q1=FileExchange_quaternionmul(Qx,Qz);  
    Q2=FileExchange_quaternionmul(Qy,Q1);
    
elseif(strcmpi(order,'yzx')==1)
   % disp('order is y-->z-->x');
    Q1=FileExchange_quaternionmul(Qz,Qx);  
    Q2=FileExchange_quaternionmul(Qy,Q1);
    
elseif(strcmpi(order,'zxy')==1)
    %disp('order is z-->x-->y');
    Q1=FileExchange_quaternionmul(Qx,Qy);  
    Q2=FileExchange_quaternionmul(Qz,Q1);
    
elseif(strcmpi(order,'zyx')==1) % same as matlab quatdemo
    %disp('order is z-->y-->x');
    Q1=FileExchange_quaternionmul(Qy,Qx);  
    Q2=FileExchange_quaternionmul(Qz,Q1); 
else
